    ...
    <>
      ...
      >
        ...
        <div className="relative max-w-7xl mx-auto w-full">
          <p className="font-display font-semibold uppercase mb-3" style={{ color: 'oklch(0.79 0.19 78)', fontSize: '0.7rem', letterSpacing: '0.22em' }}>Rolling Admissions · No GPA Minimum</p>
          <h1 className="font-display font-black text-white leading-none mb-4" style={{ fontSize: 'clamp(2.4rem, 6vw, 4.5rem)', letterSpacing: '-0.025em' }}>APPLY TO LAWSON STATE</h1>
          ...
          <div className="flex flex-wrap gap-3 mt-6">
            <Link href="https://lawsonstate.edu/admissions/apply" className="press btn-shimmer hero-cta-gold font-bold px-6 py-3 rounded-lg inline-block" style={{ background: 'oklch(0.79 0.19 78)', color: 'oklch(0.11 0.03 261)', fontSize: '0.875rem' }}>
              Start My Application
            </Link>
            <Link href="/financial-aid" className="press hero-cta-ghost font-semibold px-6 py-3 rounded-lg text-white inline-block" style={{ border: '1.5px solid oklch(1 0 0 / 0.25)', fontSize: '0.875rem' }}>
              Financial Aid Info
            </Link>
          </div>
        </div>
      ...
      <section className="py-20 px-6" style={{ background: 'white' }}>
        <div className="max-w-7xl mx-auto">
          <div className="mb-12 scroll-reveal">
            <p className="font-display font-semibold uppercase mb-3" style={{ color: 'oklch(0.79 0.19 78)', fontSize: '0.68rem', letterSpacing: '0.2em' }}>How to Apply</p>
            <h2 className="font-display font-black leading-none text-lscc-ink" style={{ fontSize: 'clamp(1.8rem, 4vw, 3rem)', letterSpacing: '-0.025em' }}>4 STEPS TO ENROLLMENT</h2>
          </div>
          <div className="relative">
            ...
            <div className="stagger-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {STEPS.map((s) => (
                <div key={s.step} className="card-lift rounded-2xl p-6 shadow-card" style={{ background: 'oklch(0.97 0.015 263)', border: '1px solid oklch(0.92 0.01 263)' }}>
                  <div className="w-10 h-10 rounded-full flex items-center justify-center mb-4 font-display font-black" style={{ background: 'oklch(0.79 0.19 78)', color: 'oklch(0.11 0.03 261)', fontSize: '0.82rem' }}>
                    {s.step}
                  </div>
                  <h3 className="font-display font-bold mb-2 text-lscc-ink leading-snug" style={{ fontSize: '0.95rem' }}>{s.title}</h3>
                  <p className="text-lscc-muted leading-relaxed mb-4" style={{ fontSize: '0.84rem' }}>{s.desc}</p>
                  {s.link && (
                    <Link href={s.link.href} className="link-reveal font-semibold" style={{ fontSize: '0.82rem', color: 'oklch(0.22 0.17 261)' }}>
                      {s.link.label}
                    </Link>
                  )}
                </div>
      ...
      <section className="py-20 px-6" style={{ background: 'oklch(0.14 0.06 263)' }}>
        <div className="max-w-7xl mx-auto">
          <div className="mb-10 scroll-reveal">
            <p className="font-display font-semibold uppercase mb-3" style={{ color: 'oklch(0.79 0.19 78)', fontSize: '0.68rem', letterSpacing: '0.2em' }}>Admissions Requirements</p>
            <h2 className="font-display font-black leading-none text-white" style={{ fontSize: 'clamp(1.8rem, 4vw, 3rem)', letterSpacing: '-0.025em' }}>FIND YOUR PATH</h2>
          </div>
            ...
            {TYPES.map(t => (
              <div key={t.id} className="rounded-2xl p-7" style={{ background: 'oklch(0.19 0.08 263)', border: '1px solid oklch(1 0 0 / 0.07)' }}>
                ...
                <ul className="space-y-3 mb-6">
                  {t.items.map(item => (
                    <li key={item} className="flex items-start gap-2.5" style={{ fontSize: '0.85rem', color: 'oklch(1 0 0 / 0.55)' }}>
                      <span style={{ color: 'oklch(0.79 0.19 78)', fontSize: '0.7rem', marginTop: '0.2rem', flexShrink: 0 }}>✓</span>
                      {item}
                    </li>
                  ))}
                </ul>
                <Link
                  ...
                  style={{ background: 'oklch(0.79 0.19 78)', color: 'oklch(0.11 0.03 261)', fontSize: '0.84rem' }}
                >
                  {t.cta} →
                </Link>
              </div>
      ...
      <section className="py-20 px-6" style={{ background: 'white' }}>
        <div className="max-w-7xl mx-auto scroll-reveal">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            <div>
              <p className="font-display font-semibold uppercase mb-3" style={{ color: 'oklch(0.79 0.19 78)', fontSize: '0.68rem', letterSpacing: '0.2em' }}>Need Help?</p>
              <h2 className="font-display font-black leading-none mb-6 text-lscc-ink" style={{ fontSize: 'clamp(1.8rem, 4vw, 2.8rem)', letterSpacing: '-0.025em' }}>CONTACT ADMISSIONS</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {CONTACT.map(c => (
                  <div key={c.label} className="rounded-xl p-4" style={{ background: 'oklch(0.97 0.015 263)', border: '1px solid oklch(0.92 0.01 263)' }}>
                    <div className="font-semibold uppercase mb-0.5" style={{ fontSize: '0.6rem', letterSpacing: '0.14em', color: 'oklch(0.79 0.19 78)' }}>{c.label}</div>
                    <div className="font-medium text-lscc-ink" style={{ fontSize: '0.9rem' }}>{c.value}</div>
                  </div>
                ))}
              </div>
            </div>
            <div>
              <p className="font-display font-semibold uppercase mb-3" style={{ color: 'oklch(0.79 0.19 78)', fontSize: '0.68rem', letterSpacing: '0.2em' }}>Office Hours</p>
              <h2 className="font-display font-black leading-none mb-6 text-lscc-ink" style={{ fontSize: 'clamp(1.8rem, 4vw, 2.8rem)', letterSpacing: '-0.025em' }}>WHEN WE'RE OPEN</h2>
              <div className="space-y-3">
                {[
                  ['General Office Hours', 'Monday–Friday: 8:00 AM – 5:00 PM'],
                  ['Admissions (Extended)', 'Monday–Thursday: 8:00 AM – 6:30 PM\nFriday: 8:00 AM – 5:00 PM'],
                ].map(([label, hours]) => (
                  <div key={label} className="rounded-xl p-5" style={{ background: 'oklch(0.97 0.015 263)', border: '1px solid oklch(0.92 0.01 263)' }}>
                    <div className="font-semibold mb-1 text-lscc-ink" style={{ fontSize: '0.88rem' }}>{label}</div>
                    <div className="text-lscc-muted whitespace-pre-line" style={{ fontSize: '0.84rem' }}>{hours}</div>
                  </div>
                ))}
              </div>
              <div className="mt-6">
