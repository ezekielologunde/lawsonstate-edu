...
export default function ProgramCatalog() {
  ...
  return (
    <div>
      ...
      <div className="mb-8 space-y-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            style={{
              background: 'oklch(0.22 0.16 261)', border: '1.5px solid oklch(1 0 0 / 0.15)',
          ...
          {query && (
            <button onClick={() => setQuery('')}
              ...
              style={{ color: 'oklch(1 0 0 / 0.40)', fontSize: '1.1rem', lineHeight: 1 }}
            ...
            return (
              <button key={t.id} onClick={() => { setCat(t.id as ProgramCategory | 'all'); setShowAll(false) }}
                ...
                style={{
                  ...
                  background: active ? 'oklch(0.79 0.19 78)' : 'oklch(0.22 0.16 261)',
                  color:      active ? 'oklch(0.11 0.03 261)' : 'oklch(1 0 0 / 0.70)',
            ...
            return (
              <button key={t.id} onClick={() => { setDegree(t.id as DegreeType | 'all'); setShowAll(false) }}
                ...
                style={{
                  ...
                  background: active ? 'oklch(0.30 0.20 261)' : 'transparent',
                  color:      active ? 'white'              : 'oklch(1 0 0 / 0.45)',
                  border:     '1px solid ' + (active ? 'oklch(0.42 0.22 261)' : 'oklch(1 0 0 / 0.10)'),
                }}>
                {t.label}
              </button>
            )
        ...
        {/* Count */}
        <p style={{ fontSize: '0.78rem', color: 'oklch(1 0 0 / 0.45)' }}>
      ...
      {/* Program grid */}
      {filtered.length === 0 ? (
        <div className="text-center py-20 rounded-2xl" style={{ background: 'oklch(0.18 0.14 261)' }}>
          <p style={{ fontSize: '1.1rem', color: 'oklch(1 0 0 / 0.55)', marginBottom: '0.5rem' }}>
            No programs match your search.
          </p>
          <button onClick={() => { setQuery(''); setCat('all'); setDegree('all') }}
            className="press font-semibold"
            style={{ fontSize: '0.82rem', color: 'oklch(0.79 0.19 78)', textDecoration: 'underline' }}>
            Clear filters
          </button>
        </div>
      ) : (
            ...
            {visible.map(p => {
              ...
              return (
                <Link key={p.id} href={p.href}
                  ...
                  style={{ background: 'oklch(0.20 0.15 261)' }}>
                  ...
                  <div className="flex flex-col flex-1 p-5">
                    ...
                    <div className="flex items-center gap-2 flex-wrap mb-3">
                      <span className="font-bold rounded-full px-2.5 py-0.5"
                        style={{ fontSize: '0.65rem', letterSpacing: '0.10em', textTransform: 'uppercase', background: 'oklch(1 0 0 / 0.08)', color: 'oklch(1 0 0 / 0.55)' }}>
                      ...
                      {p.transferable && (
                        <span className="font-bold rounded-full px-2.5 py-0.5"
                          style={{ fontSize: '0.65rem', letterSpacing: '0.10em', textTransform: 'uppercase', background: 'oklch(0.62 0.18 280 / 0.18)', color: 'oklch(0.80 0.15 280)' }}>
                          Transfer
                        </span>
                      )}
                    </div>
                    ...
                    {/* Program name */}
                    <h3 className="font-display font-black text-white leading-tight mb-2"
                      style={{ fontSize: 'clamp(1rem, 1.5vw, 1.15rem)', letterSpacing: '-0.02em', flex: 'none' }}>
                      {p.name}
                    </h3>

                    {/* Description */}
                    <p className="flex-1 leading-relaxed mb-4"
                      style={{ fontSize: '0.80rem', color: 'oklch(1 0 0 / 0.60)', lineHeight: 1.65 }}>
                      {p.description}
                    </p>
                    ...
                    <div className="flex items-end justify-between gap-3 mt-auto">
                      <div className="flex flex-col gap-1.5">
                        ...
                        <div className="flex items-center gap-1.5">
                          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden
                            style={{ color: 'oklch(1 0 0 / 0.30)', flexShrink: 0 }}>
                            <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
                          </svg>
                          <span style={{ fontSize: '0.70rem', color: 'oklch(1 0 0 / 0.45)', fontWeight: 600 }}>
                            {p.duration}
                          </span>
                        </div>
                        {/* Delivery modes */}
                        <div className="flex flex-wrap gap-1">
                          {p.deliveryModes.map(m => (
                            <span key={m} className="rounded px-1.5 py-0.5"
                              style={{ fontSize: '0.62rem', fontWeight: 600, letterSpacing: '0.04em', background: 'oklch(1 0 0 / 0.06)', color: 'oklch(1 0 0 / 0.40)' }}>
                              {DELIVERY_LABEL[m]}
                            </span>
                          ))}
                        </div>
                      ...
                      {p.jobPlacementRate && (
                        <div className="flex flex-col items-end shrink-0">
                          ...
                          <span style={{ fontSize: '0.58rem', color: 'oklch(1 0 0 / 0.35)', letterSpacing: '0.06em', textTransform: 'uppercase', marginTop: '1px' }}>
                            placed
                          </span>
                        </div>
                      )}
                    ...
                    <div className="mt-4 pt-4 flex items-center justify-between"
                      ...
                      <span style={{ fontSize: '0.70rem', color: 'oklch(1 0 0 / 0.35)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
                        {catLabel}
                      </span>
          ...
          {!showAll && filtered.length > 24 && (
            <div className="mt-8 text-center">
              <button onClick={() => setShowAll(true)}
                ...
                style={{ background: 'oklch(0.22 0.16 261)', border: '1.5px solid oklch(1 0 0 / 0.18)', color: 'white', fontSize: '0.9rem' }}>
