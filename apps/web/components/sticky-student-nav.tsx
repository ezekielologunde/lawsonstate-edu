...
const STUDENT_ACTIONS = [
  { label: 'Student Portal', href: '/student-portal', primary: true },
...
export default function StickyStudentNav() {
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 z-40 shadow-lg lg:top-0 lg:bottom-auto"
      style={{
        background: 'white',
    ...
    >
      <div className="max-w-7xl mx-auto px-4 py-3 flex gap-2 overflow-x-auto scrollbar-hide">
        ...
        {STUDENT_ACTIONS.map(({ label, href, primary }) => (
          <Link
            key={label}
            href={href}
            className="press shrink-0 px-4 py-2.5 rounded-full font-semibold whitespace-nowrap transition-colors text-sm"
            style={{
